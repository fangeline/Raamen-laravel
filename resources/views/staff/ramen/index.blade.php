                ...
                @foreach ($ramen as $item)
                    <tr>
                        ...
                        <td style="text-align: center">
                            <form method="post">
                                <a class="- btn btn-outline-secondary rounded small" style="min-width: 70px" href="{{ route('editRamen', $item['id']) }}">Edit</a>
                                <a class="- btn btn-outline-secondary rounded small" style="min-width: 70px" href="{{ route('deleteRamen', $item['id']) }}">Delete</a>
                            </form>
                        </td>
                    </tr>
                @endforeach
...
<script>
    ...
    input.addEventListener("keypress", function(event) {
        ...
        if (event.keyCode == 13) {
            window.location.href = "{{ url('/ramen/?keyword=') }}" + $("#keyword").val();
        }
    });
</script>
