                        ...
                        @if ($item['staff_id'] == 0)
                            ...
                            <td style="text-align: center">
                                <form method="post">
                                    ...
                                    <a class="- btn btn-outline-primary rounded small" style="min-width: 70px" href="{{ route('handleOrder.admin', $item['id']) }}">Handle</a>
                                    <a class="- btn btn-outline-secondary rounded small" style="min-width: 70px" href="{{ route('removeOrder.admin', $item['id']) }}">Remove</a>
                                </form>
                            </td>
                        @else
                            <td>Handled</td>
                            <td style="text-align: center">
                                <form method="post">
                                    <a class="- btn btn-outline-secondary rounded small" style="min-width: 70px" href="{{ route('showDetail.admin', $item['id']) }}">Details</a>
                                    <a class="- btn btn-outline-primary rounded small disabled" style="min-width: 70px" href="{{ route('handleOrder.admin', $item['id']) }}">Handle</a>
                                    <a class="- btn btn-outline-secondary rounded small disabled" style="min-width: 70px" href="{{ route('removeOrder.admin', $item['id']) }}">Remove</a>
                                </form>
                            </td>
                        @endif
                    </tr>
...
<script>
    ...
    input.addEventListener("keypress", function(event) {
        ...
        if (event.keyCode == 13) {
            window.location.href = "{{ url('/adminQueue/?keyword=') }}" + $("#keyword").val();
        }
    });
</script>
