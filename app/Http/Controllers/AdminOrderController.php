...
class AdminOrderController extends Controller
{
    ...
    {
        if(!Detail::findOrFail($id)->exists())
        {
            return redirect()->route('showDetail.admin')->with('error_message', 'Data does not exists, please refresh the page.');
        }

        $detail = Detail::findOrFail($id);
        $detail->destroy($id);

        return redirect()->route('showQueue.admin')->with('success_message', 'Success removing detail for this order!');
